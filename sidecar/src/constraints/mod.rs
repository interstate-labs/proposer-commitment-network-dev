...
mod tests {
    use super::*;
    use crate::{
        config::ChainConfig, keystores::BLSSig, utils::create_random_bls_secretkey, BLS_DST_PREFIX,
    };
    ...
    fn test_constraints_message_build() {
        ...
        let constraint = ConstraintsMessage::from_tx(
            ECBlsPublicKey::try_from(signer.sk_to_pk().to_bytes().as_ref()).unwrap(),
            165,
            tx,
        );
