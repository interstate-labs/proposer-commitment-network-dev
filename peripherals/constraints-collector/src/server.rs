...
use crate::{
    ...
    SignedConstraints, VersionedValue,
};
...
pub async fn run_constraints_collector(port: u16, cb_url: String) {
    let collector = Arc::new(ConstraintsCollector::new(
        cb_url.parse().expect("Valid URL"),
    ));
...
async fn get_header(
    State(collector): State<Arc<ConstraintsCollector>>,
    Path(params): Path<GetHeaderParams>,
) -> Result<Json<VersionedValue<SignedBuilderBid>>, CollectorError> {
...
) -> Result<Json<GetPayloadResponse>, CollectorError> {
...
) -> Result<StatusCode, CollectorError> {
