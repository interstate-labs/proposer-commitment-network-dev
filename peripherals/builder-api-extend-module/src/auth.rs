use axum::{
    body::Body,
    extract::Request, middleware::Next, response::{Html, Response}, Json
};
...
pub async fn signature_auth_handler(
    req: Request,
    next: Next,
    ...
    let address: Address = params.pubkey.parse()
    ...
    let recovered_address = signature.recover(message_hash)
        .or_else(|_err| Err(BuilderApiError::InvalidParams("Pubkey recovery failed".to_string())))?;
...
pub fn encode_jwt(pubkey: String) -> Result<String, BuilderApiError> {
    let expiry = dotenv::var("JWT_EXPIRY_HOURS")
        .or_else(|_err| Err(BuilderApiError::ENVError("Failed to load env variables".to_string())))?;
    ...
    encode(
        ...
        .or_else(|_err| Err(BuilderApiError::ENVError("Failed to generate JWT".to_string())))
}
...
pub fn decode_jwt(token: String) -> Result<jsonwebtoken::TokenData<Client>, BuilderApiError> {
    ...
    decode(
        ...
        .or_else(|_err| Err(BuilderApiError::JWTError("Failed to decode JWT".to_string())))
}


pub async fn auth_response(Json(authbody):Json<AuthRequest>) -> Result<Json<String>, BuilderApiError> {
