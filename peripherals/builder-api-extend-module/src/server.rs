...
use crate::{
    auth::{auth_response, jwt_auth_handler, signature_auth_handler, whitelisted_route},
    builder::{GetHeaderParams, GetPayloadResponse, SignedBuilderBid, VersionedValue},
...
use crate::{
    builder::{GET_HEADER_PATH, GET_PAYLOAD_PATH, REGISTER_VALIDATORS_PATH, STATUS_PATH},
    extender::Extender,
};
...
pub async fn run_builder_extend_modular(port: u16, urls: Vec<String>) {
    ...
    let router = Router::new()
        ...
        .route(REGISTER_VALIDATORS_PATH, post(register_validators))
        .route(GET_HEADER_PATH, get(get_header))
        .route(GET_PAYLOAD_PATH, post(get_payload))
        .with_state(extender)
        .route(
            "/auth_route",
            post(auth_response).layer(middleware::from_fn(signature_auth_handler)),
        )
        .route(
            "/protected_route",
            get(whitelisted_route).layer(middleware::from_fn(jwt_auth_handler)),
        );
...
async fn get_header(
    State(extender): State<Arc<Extender>>,
    ...
    match extender.get_header(params).await {
        Ok(header) => return Ok(Json(header)),
        Err(err) => {
...
async fn register_validators(
    State(extender): State<Arc<Extender>>,
    Json(registors): Json<Vec<SignedValidatorRegistration>>,
) -> Result<StatusCode, BuilderApiError> {
    tracing::debug!("handling REGISTER_VALIDATORS_REQUEST");
    match extender
        .register_validators(registors)
        .await
        .map(|_| StatusCode::OK)
        .map_err(|e| {
