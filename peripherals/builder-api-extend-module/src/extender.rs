use crate::{
    builder::{
        BuilderClient, GetHeaderParams, GetPayloadResponse, SignedBuilderBid, VersionedValue,
    },
...
impl Extender {
    pub fn new(urls: Vec<String>) -> Extender {
        ...
        for url in urls {
            extender
                .builders
                .push(BuilderClient::new(Url::parse(&url).unwrap()));
        }
    ...
    pub async fn status(&self) -> Result<StatusCode, BuilderApiError> {
        ...
        for builder in &self.builders {
            ...
            handles.push(tokio::spawn(async move { nbuilder.status().await }));
        }
