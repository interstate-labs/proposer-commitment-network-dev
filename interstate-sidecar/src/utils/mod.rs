...
pub async fn send_sidecar_info(
    ...
    let response = client.post(server_url.clone()).json(&data).send().await?;
    ...
    if response.status() != StatusCode::OK {
        ...
        tracing::error!(
            ?error,
            "Failed to send sidecar data to server {}",
            server_url.as_str()
        );
    }
    tracing::info!(
        "Sent successfully sidecar data to the server {}",
        server_url.as_str()
    );
    Ok(())
}
